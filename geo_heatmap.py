...
from utils import is_text_based_browser, timestamp_in_range, date_in_range


class Generator:
    def __init__(self):
        self.stats = {"Data points": 0}
    ...
    def load_json_data(self, json_file, date_range):
        ...
        with ProgressBar(max_value=len(data["locations"]), widgets=w) as pb:
            for i, loc in enumerate(data["locations"]):
                if "latitudeE7" not in loc or "longitudeE7" not in loc:
                    continue
                coords = (
                    round(loc["latitudeE7"] / 1e7, 6),
                    round(loc["longitudeE7"] / 1e7, 6),
                )

                if timestamp_in_range(loc["timestampMs"], date_range):
                    self.update_coordinate(coords)
                pb.update(i)

    def stream_json_data(self, json_file, date_range):
        ...
        max_value_est = sum(1 for _ in json_file) / 13
        json_file.seek(0)
    ...
    def load_kml_data(self, file_name, date_range):
        ...
        with ProgressBar(max_value=len(gxtrack), widgets=w) as pb:
            for i, number in enumerate(gxtrack):
                ...
                if date_in_range(date[:10], date_range):
                    self.update_coordinate(coords)
                pb.update(i)

    def load_gpx_data(self, file_name, date_range):
    ...
    def load_zip_data(self, file_name, date_range):
        """
        ...
        (data_path,) = fnmatch.filter(
            namelist, "Takeout/{name}/{name}.*".format(name=name)
        )
        print("Reading location data file from zip archive: {!r}".format(data_path))

        if data_path.endswith(".json"):
            with zip_file.open(data_path) as read_file:
                self.load_json_data(read_file, date_range)
        elif data_path.endswith(".kml"):
            with zip_file.open(data_path) as read_file:
                self.load_kml_data(read_file, date_range)
        else:
            raise ValueError(
                "unsupported extension for {!r}: only .json and .kml supported".format(
                    file_name
                )
            )
    ...
    def generate_map(self, settings):
        ...
        map_data = [
            (coords[0], coords[1], magnitude)
            for coords, magnitude in self.coordinates.items()
        ]

        # Generate map
        m = folium.Map(
            location=self.max_coordinates, zoom_start=zoom_start, tiles=tiles
        )
    ...
    def run(self, data_files, output_file, date_range, stream_data, settings):
        ...
        for i, data_file in enumerate(data_files):
            print(
                "\n({}/{}) Loading data from {}".format(
                    i + 1, len(data_files) + 2, data_file
                )
            )
            if data_file.endswith(".zip"):
                self.load_zip_data(data_file, date_range)
            elif data_file.endswith(".json"):
                with open(data_file) as json_file:
                    if stream_data:
                        self.stream_json_data(json_file, date_range)
                    else:
                        self.load_json_data(json_file, date_range)
            elif data_file.endswith(".kml"):
                self.load_kml_data(data_file, date_range)
            elif data_file.endswith(".gpx"):
                self.load_gpx_data(data_file, date_range)
            else:
        ...
        print(
            "\n({}/{}) Generating heatmap".format(
                len(data_files) + 1, len(data_files) + 2
            )
        )
        m = self.generate_map(settings)
        print(
            "\n({}/{}) Saving map to {}\n".format(
                len(data_files) + 2, len(data_files) + 2, output_file
            )
        )
        m.save(output_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="Any of the following files:\n"
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="Path of heatmap HTML output file.",
    ...
    parser.add_argument(
        ...
        dest="stream",
        action="store_true",
        help="Option to iteratively load data.",
    )
    parser.add_argument(
        "--map",
        "-m",
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        help="The initial zoom level for the map. (default: %(default)s)",
        default=6,
    )
    parser.add_argument(
        "-r",
        ...
        type=int,
        required=False,
        help="The radius of each location point. (default: %(default)s)",
        default=7,
    )
    parser.add_argument(
        "-b",
        ...
        type=int,
        required=False,
        help="The amount of blur. (default: %(default)s)",
        default=4,
    )
    parser.add_argument(
        "-mo",
        ...
        type=float,
        required=False,
        help="The minimum opacity of the heatmap. (default: %(default)s)",
        default=0.2,
    )
    parser.add_argument(
        "-mz",
        ...
        type=int,
        required=False,
        help="The maximum zoom of the heatmap. (default: %(default)s)",
        default=4,
    )
    ...
    if not is_text_based_browser(webbrowser.get()):
        try:
            print("[info] Opening {} in browser".format(output_file))
            webbrowser.open("file://" + os.path.realpath(output_file))
        except webbrowser.Error:
            print(
                "[info] No runnable browser found. Open {} manually.".format(
                    output_file
                )
            )
            print(
                '[info] Path to heatmap file: "{}"'.format(os.path.abspath(output_file))
            )
